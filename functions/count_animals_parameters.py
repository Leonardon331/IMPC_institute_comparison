...
def count_animals_parameters(gene_symbols: list):
    ...
    ## combined animal and parameter bar plot with every gene! ##
    print("make combined animal and parameter at center bar plot for all genes")
    ...
    subplot_columns = 2
    subplot_rows = math.ceil(len(gene_symbols)/2)
    ...
    for gene_symbol in gene_symbols: 
        ...
        d = counts[:len(counts)-3]
        
        ax = ax_list[i]
        
        #remove to many subplots
        if i > len(gene_symbols)-1:
            ax.remove()
            continue
        
        d.plot(x="phenotyping_center", y=["animal_count", "parameter_count"], kind="bar", width=0.8, ax=ax, legend=False)
        ax.set_xticklabels(d.phenotyping_center)
        ax.tick_params(axis='x', labelrotation = 90)
        ax.set_title(f"{gene_symbol}", fontsize = 18)
        ax.set_xlabel("Phenotyping Center", fontsize=14)
        ax.set_ylabel("Count", fontsize=14)
        for j in ax.containers:
            ax.bar_label(j,fontsize = 10) #add numbers on top of bars
        if i >= center_count:
            ax.remove()
        i+=1
    
    #legend
    fig.legend(labels=["Animals", "Parameters"], bbox_to_anchor=(1,0.95), loc='upper left', fontsize=14)
    
    fig.suptitle('Number of animals and parameters across centers and genes', size = 26, ha = 'center',  y=1.001)
    plt.tight_layout()
    
    plt.savefig(r'./overall_analysis/genes_counts/barplot_gene_counts_combined.png', dpi = 500, bbox_inches='tight')
    print("---------------------")
    
    ## plot only parameters across centers and genes ##
    #make stacked bar chart with IMPC procedures and center own parameters
       
#%%
#call the function

count_animals_parameters(gene_symbols = ["Ap4e1", "Dbn1", "Nxn", "Prkab1"])
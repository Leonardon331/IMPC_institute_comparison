...
def compare_sd(BW_age_in_weeks: int = 14, min_centers: int = 4, sd_calculated: bool = False, id_plots_done: bool = False, save_memory: bool = True):
    ...
    if id_plots_done == False:
        ...
        for parameter_stable_id in set(control_stats_norm.parameter_stable_id):
            ...
            if parameter_stable_id == "IMPC_BWT_008_001":
                bar.figure.suptitle(f'Normalized wt standard deviation across centers for {parameter_name} ({parameter_stable_id}), age: {BW_age_in_weeks} weeks', size = 18, ha = 'center')
                #bug: ha/horizontalaligment= 'right' ----> 'left'
        
            else:
                bar.figure.suptitle(f'Normalized wt standard deviation across centers for {parameter_name} ({parameter_stable_id})', size = 18, ha = 'center')
                #bug: ha/horizontalaligment= 'right' ----> 'left'
            ...
            if parameter_stable_id == "IMPC_BWT_008_001":
                ...
                plt.savefig(f"{save}/bar_plot_{parameter_stable_id}_normalized_wt_standard_deviation_{BW_age_in_weeks}_weeks.png", dpi = 500, bbox_inches='tight')
                #whole path for plot saving important
                
            else:
                plt.savefig(f"{save}/bar_plot_{parameter_stable_id}_normalized_wt_standard_deviation_.png", dpi = 500, bbox_inches='tight')
                #whole path for plot saving important
    ...
    bar.figure.suptitle('Normalized wt standard deviation sex mean over all parameters', size = 18, ha = 'center')
        #bug: ha/horizontalaligment= 'right' ----> 'left'
    
    for ax in bar.axes.flat:
        ax.tick_params(axis='x', labelrotation = 90)
        ax.set_xlabel("Phenotyping Center", fontsize=14)
        ax.set_ylabel("Normalized Standard Deviation Mean", fontsize=14)
    
        #for i in bar.containers: ## similar to that
            ###bar.bar_label(i,)
    
    plt.savefig(f"{save}/bar_plot_normalized_wt_standard_deviation_sex_mean.png", dpi = 500, bbox_inches='tight')
    #whole path for plot saving important
    ...
    bar.figure.suptitle('Normalized wt standard deviation mean over all sexes and parameters', size = 18, ha = 'center')
        #bug: ha/horizontalaligment= 'right' ----> 'left'
    ...
    plt.savefig(f"{save}/bar_plot_normalized_wt_standard_deviation_mean.png", dpi = 500, bbox_inches='tight')

...
#%%
def ANOVA_center(gene_symbols: list, parameter_stable_ids: list, post_hoc_method: str = "ttest", p_adjust: str =  "fdr_bh", plot_type: str = "box", focus_zygosity: bool = True, less_outliers: bool = False, save_memory: bool = False):
    '''
    ...
    parameter_stable_ids: list of strings, IMPC pipline parameter ids used for more specific center comparison
    ...
    -------
    None
    assignes automatically variable for dataframe with ANOVA_results and group-descriptive statistics and
    for significant ANOVAs also post hoc data frames
    ...
    for gene_symbol in gene_symbols:
        for parameter_stable_id in parameter_stable_ids:
            ...
            box.fig.suptitle(f'Knockout-WT-Ratio for {parameter_name} ({parameter_stable_id}) of {gene_symbol}', size = 18, ha = 'center')
            ...
            if less_outliers == True:
                plt.savefig(f"{path}/{plot_type}plot_{gene_symbol}_{parameter_stable_id}_kwr_{zygosity}_less_outliers.png", dpi = 500, bbox_inches='tight')
                #whole path for plot saving important
                
                results.to_csv(f'{path}/{gene_symbol}_{parameter_stable_id}_ANOVA_{zygosity}_less_outliers.csv', index=False)
                
                if save_memory == False:
                    globals()[f"{gene_symbol}_{parameter_stable_id}_ANOVA_{zygosity}_less_outliers"] = results
                    # global dynamic variable gets results assigned
                        # no need to return a variable
                    
                if len(all_post_hoc_p) != 0:
                    all_post_hoc_p.to_csv(f'{path}/{gene_symbol}_{parameter_stable_id}_post_hoc_p_{post_hoc_method}_{zygosity}_less_outliers.csv', index=False)
                    if save_memory == False:
                        globals()[f"{gene_symbol}_{parameter_stable_id}_post_hoc_p_{zygosity}_less_outliers"] = all_post_hoc_p
                
                    
            if less_outliers == False:
                plt.savefig(f"{path}/{plot_type}plot_{gene_symbol}_{parameter_stable_id}_kwr_{zygosity}_outliers_incl.png", dpi = 500, bbox_inches='tight')
                #whole path for plot saving important
                
                results.to_csv(f'{path}/{gene_symbol}_{parameter_stable_id}_ANOVA_{zygosity}_outliers_incl.csv', index=False)
                
                if save_memory == False:
                    globals()[f"{gene_symbol}_{parameter_stable_id}_ANOVA_{zygosity}_outliers_incl"] = results
                    
                if len(all_post_hoc_p) != 0:
                    all_post_hoc_p.to_csv(f'{path}/{gene_symbol}_{parameter_stable_id}_post_hoc_p_{post_hoc_method}_{zygosity}_outliers_incl.csv', index=False)
                    if save_memory == False:
                        globals()[f"{gene_symbol}_{parameter_stable_id}_post_hoc_p_{zygosity}_outliers_incl"] = all_post_hoc_p
              
                
            if save_memory == True:
                plt.clf()
                #clear current figure, only saving of figure
#%%
#call function

#reduce IDs if memory problems
...
ANOVA_center(genes, IDs, save_memory = True)